...
const Dashboard = () => {
  ...
  const [SOStatusCount, setSOStatusCount] = useState({
    active_so_count: 0,
  ...
  const fetchMonthlySale = async () => {
    try {
      const response = await PrivateAxios.get('report/total-sales-of-this-month');
      const { data } = response.data || {};
      if (data) {
        setMonthlySale({
          total_sale_amount: Math.round(data.total_sale_amount ?? 0),
  ...
  const fetchMonthlyPurchaseReport = async (page = 1) => {
    ...
    try {
      const response = await PrivateAxios.get(
        `report/monthly-sales-report?page=${page}&limit=${reportLimit}`
      );
      const { data } = response.data || {};
      if (data) {
  ...
  const fetchSOStatusCount = async () => {
    try {
      const response = await PrivateAxios.get('report/status-wise-sales-report');
      const responseData = response.data?.data || null;
      if (responseData) {
        setSOStatusCount({
          active_so_count: responseData.active_so_count,
          pending_approval_count: responseData.pending_approval_count,
          pending_dispatch_count: responseData.pending_dispatch_count,
        });
      }
    }
    catch (error) {
          ...
          <div className="row">
              ...
              <div className="card-body">
                  ...
                  >
                    <div className="card-title fs-4">
                      {monthlySale === null
                        ? '...'
                        : `₹ ${Number(monthlySale.total_sale_amount).toLocaleString('en-IN', { minimumFractionDigits: 0, maximumFractionDigits: 2 })}`}
                    </div>
                    <div className="card-text">
                      <FaDownload style={{ marginBottom: '2px' }} /> In {monthlySale === null
                        ? '...'
                        : `${MONTH_NAMES[(monthlySale.month - 1)] || monthlySale.month} ${monthlySale.year}`}
            ...
            <div className="col-xl-3 col-lg-6 col-md-6 col-sm-12">
              <div className="card danger-gredient mb-3">
                <div className="card-body">
                  <div className="card-title fs-4">{SOStatusCount.active_so_count} Active SO</div>
                  <div className="card-text"><FaShoppingCart /> Total Active SO</div>
                </div>
              </div>
            </div>
            <div className="col-xl-3 col-lg-6 col-md-6 col-sm-12">
              <div className="card success-gredient mb-3">
                <div className="card-body">
                  <div className="card-title fs-4">{SOStatusCount.pending_approval_count} Pending Approval</div>
                  <div className="card-text"><FaUsers /> Total Pending Approvals</div>
                </div>
              </div>
            </div>
            <div className="col-xl-3 col-lg-6 col-md-6 col-sm-12">
              <div className="card info-gredient mb-3">
                <div className="card-body">
                  <div className="card-title fs-4">{SOStatusCount.pending_dispatch_count} Pending Dispatch</div>
                  <div className="card-text"><FaChartPie /> Total Pending Dispatch</div>
                </div>
              </div>
            </div>
          </div>
                  ...
                  <Table responsive bordered hover className="mb-0">
                    <thead className="table-light">
                      <tr>
                        ...
                        <th>Total Completed SO</th>
                        <th className="text-end">Total Completed SO Amount</th>
                      </tr>
                    </thead>
                    <tbody>
                      {reportRows.length === 0 ? (
                        ...
                        reportRows.map((row, idx) => (
                          <tr key={`${row.year}-${row.month}-${idx}`}>
                            ...
                            <td>{row.total_so_count}</td>
                            <td className="text-end">
                              ₹ {Number(row.total_sales_amount).toFixed(2)}
                            </td>
                            <td>{row.completed_so_count}</td>
                            <td className="text-end">
                              ₹ {Number(row.completed_total_so_amount).toFixed(2)}
                            </td>
                          </tr>
                        ))
                      )}
