...
function CrateFullyBilled() {
  ...
  const TaskData = async () => {
    setIsLoading(true);
    PrivateAxios.get("purchase/purchase-billed")
      .then((res) => {
          ...
          return {
            ...
            status_return:
              item.status === 1
                ...
                : item.status === 6
                ? `<label class="badge badge-outline-quotation mb-0"><i class="fas fa-circle f-s-8 d-flex me-1"></i>Fully Billed</label>`
                : item.status === 7
...
export default CrateFullyBilled;
