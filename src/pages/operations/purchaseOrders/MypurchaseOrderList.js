...
import PurchaseOrdersStatusBar from "./PurchaseOrdersStatusBar";

function MypurchaseOrderList() {
  const { isLoading, setIsLoading, Logout, getGeneralSettingssymbol, MatchPermission } =
    UserAuth();
  ...
  const sendMailByPO = async (id) => {
    try {
      setIsLoading(true);
      const response = await PrivateAxios.post(`purchase/emailsend/${id}`);

      if (response.status === 200) {
        await PrivateAxios.post(`purchase/emailsendupdate/${id}`);
        setIsLoading(false);
  ...
  const TaskData = async () => {
    setIsLoading(true);
    PrivateAxios.get("purchase/getallpurchaseorder")
      .then((res) => {
        const transformedData = res.data.map((item, index) => ({
          ...
          confirmationDate: moment(item.order_dateline || item.expected_arrival).format(
            "DD-MM-YYYY hh:mm A"
          ),
              ...
              : item.status === 2
                ...
                : item.status === 3
                  ...
                  : item.status === 4
                    ...
                    : item.status === 5
                      ? `<label class="badge badge-outline-green mb-0"><i class="fas fa-circle f-s-8 d-flex me-1"></i>PO Approved</label>`
                      : item.status === 6
      ...
      <td>
        <div className="d-flex">
          {dataItem.status !== 7 && dataItem.status !== 8 && (
            <>
              {dataItem.advance == null ? (
                <Tooltip title="Advance Payment">
                  <Link
                    to={{ pathname: `/purchase-orders/advance/${dataItem.id}` }}
                    state={{ data: dataItem }}
              ...
              {MatchPermission(["Approve P.O"]) && (
                dataItem.mailsend_status !== 1 ? (
                  <Tooltip title="Send PO to vendor by Email">
                    <button
                  ...
                  <>
                    <Tooltip title="Resend PO to vendor by Email">
                      <button
                        onClick={() => sendMailByPO(dataItem.id)}
                        state={{ data: dataItem }}
                        className="me-1 icon-btn"
                    ...
                    <Tooltip title="Download PO">
                      <a
                        href={`${url}PO/purchase_order_${dataItem.reference}.pdf`}
                        download={`purchase_order_${dataItem.reference}.pdf`}
                        target="_blank"
        ...
        <div className="col-12">
          <div className="card">
            <div className="card-body p-0">
              ...
              <div className="d-flex justify-content-between flex-wrap align-items-center pt-2 px-3">
                <div className="bg_succes_table_head rounded_table w-100" style={{ minWidth: 0 }}>
                  <PDFExport data={data} ref={pdfExportRef}>
                    <ExcelExport data={data} ref={excelExportRef} >
                      ...
                      >
                        ...
                        <GridColumn field="reference" title="reference" filterable={false} filter="text" width="150px" />
                        <GridColumn field="confirmationDate" title="Confirmation Date" filterable={false} width="200px" />
                        <GridColumn field="vendor" title="vendor" filterable={false} filter="text" width="250px" />
                        <GridColumn field="buyer" title="buyer" filterable={false} filter="text" width="200px" />
...
export default MypurchaseOrderList;
