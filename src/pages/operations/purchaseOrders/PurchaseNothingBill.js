    ...
    PrivateAxios.get("purchase/getallpurchaseorder")
      .then((res) => {
        const transformedData = res.data.map((item, index) => ({
          ...
          : item.status === 5
          ? `<label class="badge badge-outline-purple mb-0"><i class="fas fa-circle f-s-8 d-flex me-1"></i>Nothing to Bill</label>`
          : item.status === 6
          ...
          {dataItem.status !== 7 && dataItem.status !== 8 && (
            <>
              ...
              {dataItem.mailsend_status !== 1 ? (
                <Tooltip title="Send PO by Email">
                  <button
                ...
                <Tooltip title="Resend PO by Email">
                  <button
                    onClick={() => sendMailByPO(dataItem.id)}
                    state={{ data: dataItem }}
                    className="me-1 icon-btn"
                ...
                <PDFExport data={data} ref={pdfExportRef}>
                  <ExcelExport data={data} ref={excelExportRef} >
                    ...
                    >
                      ...
                      <GridColumn field="confirmationDate" title="Confirmation Date" filterable={false} width="200px" locked={true} />
...
export default PurchaseNothingBill;
